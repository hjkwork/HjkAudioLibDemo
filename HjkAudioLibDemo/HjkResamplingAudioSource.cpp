...
HjkResamplingAudioSource::HjkResamplingAudioSource(HjkAudioSource* const inputSource,
...
HjkResamplingAudioSource::~HjkResamplingAudioSource() {}

void HjkResamplingAudioSource::setResamplingRatio(const double samplesInPerOutputSample)
{
    //@hjk debug flag
 //jassert(samplesInPerOutputSample > 0);

    const SpinLock::ScopedLockType sl(ratioLock);
    ratio = (0.0< samplesInPerOutputSample?  samplesInPerOutputSample:0.0);
}

void HjkResamplingAudioSource::prepareToPlay(int samplesPerBlockExpected, double sampleRate)
{
    const SpinLock::ScopedLockType sl(ratioLock);
...
void HjkResamplingAudioSource::flushBuffers()
{
...
void HjkResamplingAudioSource::releaseResources()
{
    input->releaseResources();
    buffer.setSize(numChannels, 0);
}

void HjkResamplingAudioSource::getNextAudioBlock(const HjkAudioSourceChannelInfo& info)
{
    const ScopedLock sl(callbackLock);
    ...
    while (sampsNeeded > sampsInBuffer)
    {
        ...
        HjkAudioSourceChannelInfo readInfo(&buffer, endOfBufferPos, numToDo);
...
void HjkResamplingAudioSource::createLowPass(const double frequencyRatio)
{
...
void HjkResamplingAudioSource::setFilterCoefficients(double c1, double c2, double c3, double c4, double c5, double c6)
{
...
void HjkResamplingAudioSource::resetFilters()
{
    if (filterStates != nullptr)
        filterStates.clear((size_t)numChannels);
}

void HjkResamplingAudioSource::applyFilter(float* samples, int num, FilterState& fs)
{
    while (--num >= 0)
    {
